...
function serializeTxID(transactionID) {
  const accountId = new AccountID(transactionID.accountID).toString();
  const { seconds, nanos } = transactionID.transactionValidStart;
  return `${accountId}-${seconds.toString()}-${padNS(nanos.toString())}`;
}
